...
class ChatService:
    ...
    async def answer(
        ...
        with trace.step("load_profile_photo_context"):
            profile_photo_context = self.profile_photo_service.context_for_user(user_id) if self.profile_photo_service else []
        ...
        try:
            ...
            user_ai_request_payload = {
                ...
                "request_contract": {
                    ...
                    "user_scoped": "profile, Telegram profile photo descriptions, relevant memories, conversation context, current user message",
                    "memory_count": len(memories),
                    "profile_photo_count": len(profile_photo_context),
                },
            }
    ...
    def _build_messages(
        ...
        payload = {
            ...
            "user_profile_photos": profile_photo_context or [],
    ...
    async def _attach_requested_image_if_needed(self, result: GroqResult, messages: list[dict[str, str]]) -> GroqResult:
        ...
        try:
            selection = await asyncio.to_thread(
                ...
                image_request=request_payload,
                image_catalog=catalog,
            )
        except Exception:
            logger.exception("image_selection_failed")
            return result
        valid_ids = {str(item.get("id") or "").strip() for item in catalog if str(item.get("id") or "").strip()}
        image_id = str(selection.image_id or "").strip()
